...
(defn -main
  ...
  (let [results (run-all-scenarios)]
    ...
    (doseq [{:keys [scenario result expected pass?]} (:scenarios results)]
      (println (format "[%s] %s" (if pass? "✓" "✗") scenario))
      (println (format "  Expected: %s" expected))
      (println (format "  Decision: %s" (:decision result)))
      (println (format "  Action: %s" (get (phase/apply-phase 1 result) :action)))
      (println ""))
    (println (format "Results: %d/%d passed" (:passed results) (:total-scenarios results)))
